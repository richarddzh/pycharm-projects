...
class HtmlRender:
    ...
    def align_children_grid(self, elem: HtmlElement, children: Array2D[HtmlElement], font_size):
        ...
        for i in range(0, children.height):
            ...
            for j in range(0, children.width):
                child = children.get(i, j)  # type: HtmlElement
                if child is not None:
                    if child.height > rows[i].height:
                        rows[i].height = child.height
                    if child.baseline > rows[i].baseline:
                        rows[i].baseline = child.baseline
                    if child.width > columns[j].width:
                        columns[j].width = child.width
            for j in range(0, children.width):
                child = children.get(i, j)  # type: HtmlElement
                if child is not None:
                    child.y = rows[i].y + rows[i].baseline - child.baseline
